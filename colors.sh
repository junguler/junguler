...
for j in {1..10} ; do for i in gr{1..3} ; do printf -v hex '%02X' $((#i)) ; printf "%02x%02x%02x" $((RANDOM%256)) $((RANDOM%256)) $((RANDOM%256)) > c1.txt ; printf "%02x%02x%02x" $((RANDOM%256)) $((RANDOM%256)) $((RANDOM%256)) > c2.txt ; convert -size 30x30 -define gradient:angle=$((RANDOM%360+1)) gradient:\#$(cat c1.txt)-\#$(cat c2.txt) $i-$j.jpg ; done ; done
